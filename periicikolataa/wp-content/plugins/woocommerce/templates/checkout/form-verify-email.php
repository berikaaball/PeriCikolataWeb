...
<form name="checkout" method="post" class="woocommerce-form woocommerce-verify-email" action="<?php echo esc_url( $verify_url ); ?>" enctype="multipart/form-data">
	...
	if ( $failed_submission ) {
		wc_print_notice( esc_html__( 'We were unable to verify the email address you provided. Please try again.', 'woocommerce' ), 'hata' );
	}
	?>
	<p>
		<?php
		printf(
			...
			esc_html__( 'To view this page you must verify %1$slogin%2$s or the email address associated with the order.', 'woocommerce' ),
	...
	<p class="form-row">
		<label for="email"><?php esc_html_e( 'E-mail address', 'woocommerce' ); ?>&nbsp;<span class="required">*</span></label>
